...
void bd_display_run(char *cmd, int bl_duty, int bl_on)
{
	...
	if (bl_on) {
		...
		NX_GPIO_SetPadFunction(1, 11, NX_GPIO_PADFUNC_2);
		NX_GPIO_SetOutputEnable(1, 11, CTRUE);
		NX_GPIO_SetOutputValue(1, 11, CTRUE);
	}
}

#define	UPDATE_KEY		(PAD_GPIO_B + 16)
...
extern void fs6818_lcd_select(void);
