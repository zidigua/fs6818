...
static void later_run_set_env(char *dir, char *file)
{
	...
	p = (char *)CFG_FASTBOOT_TRANSFER_BUFFER;
	p[size] = 0;
...
static int update_from_sd (char *part, char *dir, char *file)
{
	...
	/* If file is empty or NULL, just erase the part. */
	{
		...
		ret = f_interface.rx_handler((const unsigned char *)command, sizeof(command));
	}
...
//	int enable_reset = 0;
	...
	crc = get_file_crc(0, "/fs6818-android", "ubootpak.bin");
	if(crc != 0)
	{
		crc = get_file_crc(crc, "/fs6818-android", "boot.img");
		if(crc != 0)
		{
			crc = get_file_crc(crc, "/fs6818-android", "system.img");
			if(crc != 0)
			{
				if((androidcrc == 0) || (androidcrc != crc) || (force_update_flag == 1))
				{
					fboot_lcd_start();
					update_from_sd("ubootpak", "/fs6818-android", "ubootpak.bin");
					update_from_sd("boot", "/fs6818-android", "boot.img");
					update_from_sd("system", "/fs6818-android", "system.img");
					update_from_sd("userdata", "/fs6818-android", "userdata.img");
					update_from_sd("cache", "/fs6818-android", "cache.img");
					update_from_sd("recovery", "/fs6818-android", "recovery.img");
					...
					later_run_set_env("/fs6818-android", "env.txt");
					fboot_lcd_stop();
	...
	crc = get_file_crc(0, "/fs6818-qt", "ubootpak.bin");
	if(crc != 0)
	{
		crc = get_file_crc(crc, "/fs6818-qt", "boot.img");
		if(crc != 0)
		{
			crc = get_file_crc(crc, "/fs6818-qt", "qt-rootfs.img");
			if(crc != 0)
			{
				if((qtcrc == 0) || (qtcrc != crc) || (force_update_flag == 1))
				{
					fboot_lcd_start();
					update_from_sd("ubootpak", "/fs6818-qt", "ubootpak.bin");
					update_from_sd("boot", "/fs6818-qt", "boot.img");
					update_from_sd("system", "/fs6818-qt", "qt-rootfs.img");
					...
					later_run_set_env("/fs6818-qt", "env.txt");
					fboot_lcd_stop();
					do_reset(NULL, 0, 0, NULL);
					return 0;
				}
	...
	crc = get_file_crc(0, "/fs6818-ubuntu", "ubootpak.bin");
	if(crc != 0)
	{
		crc = get_file_crc(crc, "/fs6818-ubuntu", "boot.img");
		if(crc != 0)
		{
			crc = get_file_crc(crc, "/fs6818-ubuntu", "ubuntu-rootfs.tar.bz2");
			if(crc != 0)
			{
				if((ubuntucrc == 0) || (ubuntucrc != crc) || (force_update_flag == 1))
				{
					...
					update_from_sd("ubootpak", "/fs6818-ubuntu", "ubootpak.bin");
					update_from_sd("boot", "/fs6818-ubuntu", "boot.img");
					...
					later_run_set_env("/fs6818-ubuntu", "env.txt");
